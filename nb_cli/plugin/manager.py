...
class PluginManager:
    ...
    def _previous_plugins(self) -> Set[str]:
        ...
        return {
            *chain.from_iterable(
                manager.available_plugins for manager in _pre_managers
            )
        }
    ...
    def load_plugin(self, name: str) -> Optional[Plugin]:
        ...
        try:
            ...
            click.echo(f'Succeeded to import "{name}"')
            ...
            click.secho(f'Failed to import "{name}": {repr(e)}')
    ...
    def load_all_plugins(self) -> Set[Plugin]:
        ...
        return set(
            filter(
                None,
                (self.load_plugin(name) for name in self.available_plugins),
            )
        )
            ...
            for manager in reversed(_managers):
                ...
                if (
                    ...
                    module_spec.loader = PluginLoader(
                        manager, fullname, module_origin
                    )
                    return module_spec
        return
...
class PluginLoader(SourceFileLoader):
    ...
    def exec_module(self, module: ModuleType) -> None:
        ...
        for pre_plugin in reversed(parent_plugins):
            if _managers.index(pre_plugin.manager) < _managers.index(
                self.manager
            ):
        ...
        # get plugin metadata
        metadata: Optional[PluginMetadata] = getattr(
